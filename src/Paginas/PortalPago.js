...
import { setDoc, doc, deleteDoc, collection, getDocs, updateDoc, getDoc } from 'firebase/firestore';
...
const PortalPago = () => {
  ...
  useEffect(() => {
    const calcularTotal = () => {
      const totalCalculado = carrito.reduce((sum, producto) => sum + producto.precio * producto.cantidad, 0);
    ...
    return actions.order.capture().then(async (details) => {
      ...
      if (user) {
        try {
          ...
          for (const producto of carrito) {
            ...
            if (productoSnapshot.exists()) {
              ...
              if (nuevaCantidad >= 0) {
                ...
                console.log(`Cantidad actualizada para el producto: ${producto.nombre}`);
              } else {
                console.error(`No hay suficiente cantidad para el producto: ${producto.nombre}`);
              }
            } else {
          ...
          console.error('Error al guardar la venta, actualizar la cantidad de productos o generar la boleta:', error);
        }
      }
    });
  };
  ...
  return (
    <div className="portal-pago-container">
      ...
      <h3>Total: ${total} CLP (~${totalUSD} USD)</h3>
      {carrito.length > 0 && (
        <PayPalScriptProvider options={{ "client-id": "AYFWNJ7Hyl8bJRygox7jOy-GIIFdeI2n4DE0HpJHoTzUYOuptXwryPgjnd2RLiPaGAM_Xr52N1jNJ1aK" }}>
          <PayPalButtons
      ...
      {contador !== null && (
        <div className="redireccion-mensaje">
          <p>Compra realizada con éxito. Serás redirigido al inicio en {contador} segundos...</p>
        </div>
      )}
      ...
      <div id="boleta-pdf" style={{ display: 'none' }}>
        ...
        <p><strong>Cliente:</strong> {user ? user.displayName : 'Cargando...'}</p>
        <p><strong>Fecha:</strong> {new Date().toLocaleString()}</p>
        <p><strong>Total:</strong> ${total} CLP (~${totalUSD} USD)</p>
        <h3>Productos:</h3>
        <ul>
          {carrito.map((producto, index) => (
            <li key={index}>
              <p><strong>Nombre:</strong> {producto.nombre}</p>
              <p><strong>Código/UID:</strong> {producto.id}</p>
